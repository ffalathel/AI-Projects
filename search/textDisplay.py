...
class PacmanGraphics:
    ...
    def update(self, state):
        ...
        if self.agentCounter == 0:
            self.turn += 1
            if DISPLAY_MOVES:
                ghosts = [pacman.nearest_point(state.getGhostPosition(i)) for i in range(1, numAgents)]
                print("%4d) P: %-8s" % (self.turn, str(pacman.nearest_point(state.getPacmanPosition()))),'| Score: %-5d' % state.score,'| Ghosts:', ghosts)
