...
fringe.push((next_state, new_path, new_cost), new_cost)
return []
...
fringe.push((start_state, [], 0), start_heuristic) # f(n) = 0 + h(start)
while not fringe.isEmpty():
current_state, path, cost = fringe.pop()
#skip visited
if current_state in visited:
...
heuristic_cost = heuristic(next_state, problem) #h(n)
priority = new_cost + heuristic_cost #f(n) = g(n) + h(n)
