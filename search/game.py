...
class Game:
    ...
    def run( self ):
        ...
        for i in range(len(self.agents)):
            ...
            if ("register_initial_state" in dir(agent)):
                ...
                if self.catchExceptions:
                    try:
                        timed_func = TimeoutFunction(agent.register_initial_state, int(self.rules.getMaxStartupTime(i)))
                        try:
        ...
        while not self.gameOver:
            ...
            if self.catchExceptions:
                try:
                    timed_func = TimeoutFunction(agent.get_action, int(self.rules.getMoveTimeout(agentIndex)) - int(move_time))
                    try:
                        start_time = time.time()
                        if skip_action:
