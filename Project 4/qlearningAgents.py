...
class QLearningAgent(ReinforcementAgent):
    ...
    def get_q_value(self, state, action):
        ...
        """
        # return 0.0 if we haven't seen this before
    ...
    def compute_action_from_q_values(self, state):
        ...
        # get all actions with best q-value and pick random one
    ...
    def update(self, state, action, next_state, reward):
        ...
        """
        # q-learning update: q = q + alpha * (reward + gamma * max_next_q - q)
        current_q_value = self.get_q_value(state, action)
        next_state_value = self.compute_value_from_q_values(next_state)
        sample = reward + self.discount * next_state_value
        new_q_value = current_q_value + self.alpha * (sample - current_q_value)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, next_state, reward):
        ...
        """
        # update weights: w = w + alpha * error * feature
        features = self.featExtractor.getFeatures(state, action)
        current_q_value = self.get_q_value(state, action)
        next_state_value = self.compute_value_from_q_values(next_state)
        difference = (reward + self.discount * next_state_value) - current_q_value
