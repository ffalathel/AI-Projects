...
class LIFO:
    "A container with a last-in-first-out (LIFO) queuing policy, a Stack."
    def __init__(self):
        self.list = []

    def put(self,bagbogbo):
        "Push 'bagbogbo' onto the stack"
        self.list.append(bagbogbo)
...
class FIFO:
    "A container with a first-in-first-out (FIFO) queuing policy, a Queue."
    def __init__(self):
        self.list = []
...
class PriorityQueue:
    ...
    def set(self, item, priority):
        # If item already in priority queue with higher priority, set (update) its priority and rebuild the heap.
        # If item already in priority queue with equal or lower priority, do nothing.
        # If item not in priority queue, do the same thing as self.put.
...
class PriorityQueueWithFunction(PriorityQueue):
    """
    Implements a priority queue with the same put/get signature of the
    FIFO and the LIFO classes. This is designed for drop-in replacement for
    ...
    def put(self, item):
        ...
        PriorityQueue.put(self, item, self.priorityFunction(item))
