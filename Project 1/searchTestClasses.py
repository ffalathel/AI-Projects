...
def parse_heuristic(heuristicText):
    heuristic = {}
...
class GraphSearchTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        if 'heuristic' in testDict:
            self.heuristic = parse_heuristic(testDict['heuristic'])
        else:
            self.heuristic = None
...
class HeuristicTest(testClasses.TestCase):
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        cost = problem.total_action_cost(path)
        print("Problem solved")
...
class ClosestDotTest(testClasses.TestCase):
    ...
    def solution(self, searchAgents):
        ...
        gameState = pacman.GameState()
        gameState.initialize(lay, 0)
        path = searchAgents.ClosestDotSearchAgent().path_to_closest_dot(gameState)
        return path
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        h0 = searchAgents.corners_heuristic(start_state, problem)
        succs = problem.get_successors(start_state)
        # cornerConsistencyA
        for succ in succs:
            h1 = searchAgents.corners_heuristic(succ[0], problem)
            if h0 - h1 > 1:
                grades.addMessage('FAIL: inconsistent heuristic')
                return False
        heuristic_cost = searchAgents.corners_heuristic(start_state, problem)
        ...
        heuristics = []
        for state in states:
            heuristics.append(searchAgents.corners_heuristic(state, problem))
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        solution = search.astar(problem, searchAgents.corners_heuristic)
        handle.write('cost: "%d"\n' % len(solution))
        handle.write('path: """\n%s\n"""\n' % wrap_solution(solution))
        handle.close()
        return True
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        problem = searchAgents.CornersProblem(game_state)
        start_state = problem.get_start_state()
        if searchAgents.corners_heuristic(start_state, problem) > true_cost:
            grades.addMessage('FAIL: Inadmissible heuristic')
            return False
        path = search.astar(problem, searchAgents.corners_heuristic)
        print("path:", path)
